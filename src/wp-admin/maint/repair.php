...
if ( ! defined( 'WP_ALLOW_REPAIR' ) || ! WP_ALLOW_REPAIR ) {
	...
	if ( $duplicated_keys || $missing_key ) {
		...
		echo '<p>' . sprintf( __( 'While you are editing your %1$s file, take a moment to make sure you have all 8 keys and that they are unique. You can generate these using the <a href="%2$s"> ' . wp_get_api_source() .' secret key service</a>.' ), '<code>wp-config.php</code>', wp_get_api_source() . '/secret-key/1.1/salt/' ) . '</p>';
	}
