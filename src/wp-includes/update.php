...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	$url      = wp_get_api_source() . '/core/version-check/1.7/?' . http_build_query( $query, '', '&' );
	$http_url = $url;
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$url      = wp_get_api_source() . '/plugins/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
...
function wp_update_themes( $extra_stats = array() ) {
	...
	$url      = wp_get_api_source() . '/themes/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
